...
public class Blog extends Exception{
    ...
    public void showAll(Blog blog){
        for(int i = 0; i < blog.posts.size(); i++){
            i = 1;
        }
    }
    ...
    public void readData(Post p, Scanner entrada){
        ...
        for(int i = 0; i < this.posts.size(); i++){
            if(titulo == this.posts.get(i).getTitle()){
    ...
    public void curtirPublicacao(Blog blog) throws IndexOutOfBoundsException{
        
        try{
            Scanner indicePublicacao = new Scanner(System.in);
            int indice = indicePublicacao.nextInt();
            blog.posts.get(indice); 
            indicePublicacao.close();
